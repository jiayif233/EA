...
parser.add_argument('--layers', default=11, type=int, help='equivalent with N = 3')
...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        # record the parameter in decimal for every epoch
        print('model parameter file created, and the present epoch value:::', epoch)
        with open(os.path.join(args.save, "learnable_parameters_" + str(epoch) + ".txt"), "w") as file:
            file.writelines("% s\n" % data for data in learnable_parameters)
            file.close()
        # record the parameter in decimal for every epoch

        # # weights in txt format
        # with open(os.path.join(args.save, 'learnable_parameters.txt'), "w") as file:
        #     file.writelines("% s\n" % data for data in learnable_parameters)
        #     file.close()
        # # weights in txt format
