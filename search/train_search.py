...
def main(genome, epochs, search_space='micro',
    ...
    # learnable_parameters_ones_counting = counter_test.counter_ones_for_params_version_2(learnable_parameters)
    # print('learnable_parameters_ones_counting :::', learnable_parameters_ones_counting)
    # print('len(learnable_parameters) :::', len(learnable_parameters))
    ...
    for epoch in trange(epochs):
        ...
        # logging.info('learnable_parameters_ones_counting_in_epoch :: %i ,, epoch:: %i',
        #              counter_test.counter_ones_for_params_version_3(list(model.parameters())), epoch)

        # # save network parameter into a separate txt file with epoch number
        # print('model parameter file created, and the present epoch value:::', epoch)
        # with open(os.path.join(save_pth, "learnable_parameters_" + str(epoch) + ".txt"), "w") as file:
        #     file.writelines("% s\n" % data for data in list(model.parameters()))
        #     file.close()
        # # save network parameter into a separate txt file with epoch number
    ...
    with open(os.path.join(save_pth, 'log.txt'), "w") as file:
        ...
        file.write("learnable_parameters_ones_counting = {}\n".format(learnable_parameters_ones_counting))
    ...
    return {
        ...
        'params': n_params,
        'flops': n_flops,
        'learnable_parameters_ones_counting': learnable_parameters_ones_counting,
    }
...
if __name__ == "__main__":
    ...
    start = time.time()
    print(main(genome=DARTS_V2, epochs=4, save='DARTS_V2_16', seed=1, init_channels=16,
